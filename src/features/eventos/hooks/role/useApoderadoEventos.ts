...
export interface UseApoderadoEventosReturn {
  list: UseListEventosReturn;
  // Apoderado can view upcoming events for their children
  canCreate: boolean;
  canEdit: boolean;
  canDelete: boolean;
  canViewAll: boolean;
...
export function useApoderadoEventos(): UseApoderadoEventosReturn {
  const list = useList({
    fetchItems: EventosService.getProximos, // Apoderado ve eventos próximos
  ...
  return {
    list,
    canCreate: false, // Apoderado no puede crear eventos
    canEdit: false,   // Solo puede ver eventos
    canDelete: false,
